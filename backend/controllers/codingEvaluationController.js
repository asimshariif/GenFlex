 ...
 */
const runCodingEvaluator = async (inputData) => {
    return new Promise((resolve, reject) => {
        const pythonProcess = spawn('python', [
            path.join(__dirname, '..', 'utils', 'codingEvaluator.py')
        ]);
        ...
        pythonProcess.on('close', (code) => {
            if (code !== 0) {
                reject(new Error(`Coding evaluation script exited with code ${code}: ${errorData}`));
                return;
            }
...
/**
 * Evaluate coding exam submission
 * @param {Object} req - Request object
 * @param {Object} res - Response object
 */
const evaluateCodingExam = async (req, res) => {
    try {
        ...
        if (submission.examType === 'CodingExam') {
            const exam = await CodingExam.findOne({
                ...
                type: { $in: ['coding', 'complex', 'diverse'] } // Only coding-related exams
            });

            if (!exam) {
                return res.status(403).json({
                    success: false,
                    message: 'You are not authorized to evaluate this submission or this is not a coding exam'
                });
            }
            ...
            const evaluationResult = await runCodingEvaluator(evaluationData);
            ...
            return res.status(400).json({
                success: false,
                message: 'This endpoint only supports coding exam evaluation'
            });
        }
    } catch (error) {
        console.error('Error in evaluateCodingExam:', error);
        res.status(500).json({
            success: false,
 ...
 * @param {Object} res - Response object
 */
const evaluateAllCodingSubmissions = async (req, res) => {
    try {
        ...
        if (!exam) {
            return res.status(403).json({
                success: false,
                message: 'You are not authorized to evaluate submissions for this exam or this is not a coding exam'
            });
        }
        ...
        console.error('Error in evaluateAllCodingSubmissions:', error);
        res.status(500).json({
            success: false,
